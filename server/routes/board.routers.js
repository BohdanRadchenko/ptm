      ...
      await new Board({
        ...
        await Board.find().exec((err, boards) => {
          ...
          return res.status(201)
              .json({message: `New board is created`, boards})
        })
      })
    })
  } catch (e) {
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    await Board.find().exec(async (err, boards) => {
      if (err) return new Error(err)
      const accessBoards = boards.filter(
          el => el.access.includes(userId))
      console.log('GET boards Success')
      return res.status(200)
          .json({message: `GET boards Success`, boards: accessBoards})
    })
  } catch (e) {
    res.status(500)
        .json(
            {message: `ERROR: GET all Board. Something went wrong! ${e.message}`})
  }
})
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    await Board.findByIdAndDelete(id).exec(async (err, board) => {
      ...
      return res.status(200)
          .json({message: `DELETE boards Success`, boards})
    })
  } catch (e) {
    res.status(500)
        .json(
            {message: `ERROR: DELETE Board. Something went wrong! ${e.message}`})
  }
})
