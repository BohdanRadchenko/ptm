    ...
    async (req, res) => {
      try {
        ...
        if (password !== passwordAgain) {
          console.log(
              '400 - Incorrect password (pass != pass) /auth.routers')
        ...
        console.log(
            `201 - User has been created - ${email} /auth.routers`)
        res.status(201).json({
          token,
          userData: {
            id: newUser._id,
        ...
        console.log(`500 - Error POST register  /auth.routers`)
        res.status(500)
...
router.post(
    ...
    async (req, res) => {
      try {
        ...
        res.status(500)
            .json({message: `error post login ${e.message}`})
      }
    })
